...
def drawCurrentWather(weatherForecastData):
    ...
    if weatherForecastData:
        counter = 0
        y1TopMergin = 50
        y1stRowOffset = 70  # アイコンの下にテキストを配置するためのオフセット/offset to place text below icon 
        x1stRowOffset = 250
        y2TopMergin = y1stRowOffset + y1TopMergin + 210
        y2ndRowOffset = 70  # アイコンの下にテキストを配置するためのオフセット/offset to place text below icon 
        x2ndRightOffset = 100
        x2ndRowOffset = 330
        draw.text((300, 20), f"Current location {currentCityName}, {CurrentStateName}, {CurrentCountryName}", font = CustomFontSmall, fill = "#600000")
        draw.text((790, 570), f"Last updated {updateTime}", font = CustomFontSmall, fill = "#600000")
        
        draw.text((x1stRowOffset, y1TopMergin), f"{currentTime} Current Weather", font = CustomFont, fill = "#600000") #text(50,50) means the left upper corner is 50 pixcel down, 50 pixel left form bace picture's left upper corner
        ...
        draw.text((x1stRowOffset, y1stRowOffset + y1TopMergin + 30), f"Temperature         {currentTempInC}℃", font = CustomFont, fill = "#600000")
        draw.text((x1stRowOffset, y1stRowOffset + y1TopMergin + 55), f"feels like   {currentFeelsLike}℃", font = CustomFont, fill = "#600000")
        draw.text((x1stRowOffset, y1stRowOffset + y1TopMergin + 80), f"Wind Speed       {currentWindSpeed} km/h", font = CustomFont, fill = "#600000")
        draw.text((x1stRowOffset, y1stRowOffset + y1TopMergin + 105), f"Humidity          {currentHumidity}%", font = CustomFont, fill = "#600000")
        draw.text((x1stRowOffset, y1stRowOffset + y1TopMergin + 130), f"Sunrise     {sunriseToday}", font = CustomFont, fill = "#600000")
        draw.text((x1stRowOffset, y1stRowOffset + y1TopMergin + 155), f"Sunset   {sunsetToday}", font = CustomFont, fill = "#600000")
        ...
        for counter, day in enumerate(forecastDays):
            ...
            draw.text((x2ndRightOffset + margin, y2TopMergin + y2ndRowOffset + 25), f"Highest   {day['day']['maxtemp_c']}℃", font = CustomFont, fill = "#600000")
            draw.text((x2ndRightOffset + margin, y2TopMergin + y2ndRowOffset + 50), f"Lowest   {day['day']['mintemp_c']}℃", font = CustomFont, fill = "#600000")
            draw.text((x2ndRightOffset + margin, y2TopMergin + y2ndRowOffset + 75), f"Precipitation Probablity    {day['day']['daily_chance_of_rain']}%", font = CustomFont, fill = "#600000")
            draw.text((x2ndRightOffset + margin, y2TopMergin + y2ndRowOffset + 100), f"Precipitation       {day['day']['totalprecip_mm']}%", font = CustomFont, fill = "#600000")
            
        return ImageTk.PhotoImage(image)
    
    else:
        draw.text((30,30), f"Failed to get weather data", font = CustomFont, fill = "#600000")
        return ImageTk.PhotoImage(image)
